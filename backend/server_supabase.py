...
def get_current_user(credentials: HTTPAuthorizationCredentials = Depends(security)):
    ...
    user = supabase.table('users').select('*').eq('user_id', payload["user_id"]).execute()
    if not user:
...
def initialize_exercises():
    ...
    for exercise in exercises:
        if not supabase.table('exercises').select('*').eq('exercise_id', exercise["exercise_id"]).execute():
...
async def register(user_data: UserRegister):
    ...
    if supabase.table('users').select('*').eq('email', user_data.email).execute():
...
async def change_password(password_data: ChangePasswordRequest, current_user: dict = Depends(get_current_user)):
    ...
    user = supabase.table('users').select('*').eq('user_id', current_user["user_id"]).execute()
...
async def create_workout_session(
    ...
    try:
        ...
        exercise = supabase.table('exercises').select('*').eq('exercise_id', session_data.exercise_id).execute()
        if not exercise:
            raise HTTPException(status_code=404, detail="Exercise not found")
